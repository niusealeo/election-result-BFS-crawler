...
This package is a **domain-scoped** BFS (breadth-first search) crawler + download “sink” designed for election-result websites that may:
- use **Cloudflare** or other anti-bot protections (so **Postman** performs outbound requests),
- expose files via **static HTML**, **JS-injected links**, or a mixture,
...
If your package uses a direct node entrypoint, it may be:
...
### Conflated “row 0” convention
Your project convention: **row 0 is a real data row** and may also carry a “meta-schema” describing which attributes appear across rows.

So:
- rows are `0..N`
- no “blank meta row” separate from the first row
- scripts must **not double-count** “meta rows”
