...
function makeDedupeRouter(baseCfg) {
  ...
  // Reset cumulative state for a specific level.
  // This is useful if prior runs polluted the cumulative state (e.g. a bug caused
  // repeated parts to be applied incorrectly) and you want to recompute from scratch.
  //
  // Body:
  //   { level: N, cascade?: boolean }
  //
  // - cascade=false (default): clears ONLY level N state.
  // - cascade=true: clears level N and all higher levels.
  //
  // NOTE: This clears state.json only. Artifacts can be regenerated by re-running
  //       the relevant level/parts. Keeping artifacts intact can help debugging.
  r.post("/dedupe/reset-level", (req, res) => {
    try {
      ...
      // Self-aware state: treat state.json as a cache; reconcile from artifacts
      // so reruns and past buggy runs can't permanently pollute cumulative totals.
      ...
      for (const L of levels) {
        if (L === level || (cascade && L >= level)) {
          delete st.levels[String(L)];
          cleared.push(L);
        }
      }

      // If the level wasn't present, still record the intent.
      if (!cleared.includes(level)) cleared.push(level);
      ...
      appendJsonl(cfg.LOG_LEVEL_RESETS, {
        ...
        level,
        cascade,
        cleared_levels: cleared.sort((a, b) => a - b),
      });

      logEvent("RESET_LEVEL", {
      ...
      return res.json({ ok: true, level, cascade, cleared_levels: cleared.sort((a, b) => a - b) });
    } catch (e) {
